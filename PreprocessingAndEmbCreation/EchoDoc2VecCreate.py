...
with open('echo_user_Sentenses_proc.json', 'r') as fp:
    user_sentences = json.load(fp)

chosen_users = {}
for user in tqdm(user_sentences):
    sentences = []
...
model = Doc2Vec(documents, vector_size=100, window=5, min_count=3, workers=30)
...
with open(dataDirectory + 'EchoDoc2vec100.p', 'wb') as handle:
    pickle.dump(user_vectors, handle)

model.save("Echo_doc2vec.model")
