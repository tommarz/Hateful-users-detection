...
def factory(module_class_string, super_cls: type = None, **kwargs):
    ...
    logging.debug('reading class {} from module {}'.format(class_name, module_name))
    cls = getattr(module, class_name)
    # if super_cls is not None:
    #     assert issubclass(cls, super_cls), "class {} should inherit from {}".format(class_name, super_cls.__name__)
    logging.debug('initialising {} with params {}'.format(class_name, kwargs))
    obj = cls(**kwargs)
    return obj
...
def load_post_model(**post_level_kwargs):
    ...
    :return:
    """
    logging.info(f'loading preprocessing object from {post_level_kwargs["preprocessing"]["output_path"]}')
    ...
    if post_level_kwargs['kwargs']['weights_only']:
        ...
        logging.info(f'loading post model from {os.path.join(model.kwargs["paths"]["model_output"])}')
...
def predict_df(trained_dataset_name, inference_dataset_name, df):
    ...
    logging.info(f"predicting all users for dataset: {inference_dataset_name}")
...
def predict_all_users(trained_dataset_name, inference_dataset_name, user_level_path):
    ...
    if not os.path.exists(os.path.join(user_level_path, "all_users_tweets.parquet")):
        logging.info(f"reading all posts of {inference_dataset_name} dataset...")
        ...
        logging.info(f"finished reading all posts of {inference_dataset_name} dataset")
        ...
        logging.info("reading all_users_tweets.parquet file...")

        full_df = pd.read_parquet(os.path.join(user_level_path, "all_users_tweets.parquet"))
        logging.info(f"full_df shape: {full_df.shape}")
        ...
        logging.info(f"full_df shape after removing empty posts: {full_df.shape}")
        logging.info("file read.")

    # SPLITTING TO CHUNKS BY TWEETS
    chunk_size = np.minimum(1000000, len(full_df))
    logging.info(f"preprocessing in chunks of {chunk_size}...")
    logging.info(f"Length of full_df: {len(full_df)}")
    for user_range in tqdm(range(0, len(full_df), chunk_size)):
        ...
        logging.info(f"predicting users tweets; indices: {user_range} to {user_range + chunk_size - 1}...")
        y_proba = post_model.predict_proba(X_test)
        # y_proba = post_model.predict(X_test)
        ...
        logging.info(f"saving predictions to {user_level_path}")
...
def prepare_inputs_outputs(dataset_name, all_posts_probs_df_path, output_path, only_inference=False):
    ...
    :return:
    """
    logging.info(f"Preparing inputs outputs for {dataset_name} dataset...")
    # user level config
...
def run_ulm_experiment():
    ...
    if os.path.exists(os.path.join(io_path, "inputs.pkl")):
        logging.info(f"Inputs-outputs already exists for dataset {inference_data}. Loading them from {io_path}...")
    ...
    for model_type in ["nn"]:  # ,"lr", "catboost", "lightgbm", "xgboost",
        logging.info(f"Executing {model_type} model...")
